...
$outgoing_id = $_SESSION['id']; // Fetch the outgoing user's ID

// Get the search query from GET request
$search_query = isset($_GET['search']) ? $_GET['search'] : '';

// SQL query to fetch users with their latest message and match the search query
$sql = "SELECT u.id AS user_id, u.name, u.profile_img, 
       m.message, m.date_created
        FROM tbl_user AS u
        LEFT JOIN tbl_travel_companion AS tc ON u.id = tc.user_id
...
<script>
    ...
    $('#confirmDelete').on('click', function () {
        ...
        var senderId = '<?php echo $_SESSION['id']; ?>'; // Get the sender ID from PHP session

        $.ajax({
            url: 'delete-message.php', // URL to your PHP script for deletion
            type: 'POST',
            data: {
                sender_id: senderId,
                receiver_id: userId
